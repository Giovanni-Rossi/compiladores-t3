...
public class JanderSemanticoUtils {
    ...
    public static void addSemanticError(Token t, String message) {
        ...
        String linePrefix = (t != null) ? String.format("Line %d: ", line) : "Error: ";
    ...
    public static JanderType checkType(SymbolTable symbolTable, JanderParser.Exp_aritmeticaContext ctx) {
        ...
        if (ctx.termo().isEmpty()) return JanderType.INVALID;

        for (TermoContext termCtx : ctx.termo()) {
            JanderType currentTermType = checkType(symbolTable, termCtx);
            if (resultType == null) {
                resultType = currentTermType;
            } else { 
                if (areTypesIncompatible(resultType, currentTermType) || !( (resultType == JanderType.INTEGER || resultType == JanderType.REAL) && (currentTermType == JanderType.INTEGER || currentTermType == JanderType.REAL) )) {
                    ...
                    addSemanticError(ctx.op1(ctx.termo().indexOf(termCtx) -1).getStart(), "Arithmetic operation in '" + varName + "' involves incompatible types: " + resultType + " and " + currentTermType);
                    return JanderType.INVALID;
                }
            ...
            if (resultType == null) {
                resultType = currentFactorType;
                 ...
                 if (areTypesIncompatible(resultType, currentFactorType) || !( (resultType == JanderType.INTEGER || resultType == JanderType.REAL) && (currentFactorType == JanderType.INTEGER || currentFactorType == JanderType.REAL) )) {
                    addSemanticError(ctx.op2(ctx.fator().indexOf(factorCtx) -1).getStart(), "Term '" + ctx.getText() + "' contains incompatible types for multiplication/division: " + resultType + " and " + currentFactorType);
                    return JanderType.INVALID;
                }
        ...
        for (ParcelaContext parcelCtx : ctx.parcela()) {
            ...
            if (resultType == null) {
                ...
                if (areTypesIncompatible(resultType, currentParcelType) || !(resultType == JanderType.INTEGER && currentParcelType == JanderType.INTEGER) ) { 
                    addSemanticError(ctx.op3(ctx.parcela().indexOf(parcelCtx) -1).getStart(), "Factor '" + ctx.getText() + "' contains incompatible types for modulo: " + resultType + " and " + currentParcelType + ". Expected INTEGER % INTEGER.");
                    return JanderType.INVALID;
                }
